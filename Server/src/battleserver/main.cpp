...
const char * gServerName = "BattleServer";
...
BOOL CALLBACK CosonleHandler(DWORD ev)
{
	...
	case CTRL_CLOSE_EVENT:
		g_pBattleNetProxy->Final("Close Event");
		g_pBattleNetProxy->CV_Wait();
		break;
	default:
		return false;
	}
...
{
	signal(sig, SIG_IGN);
	g_pBattleNetProxy->Final("Linux Signal Close Event!");
	//	g_pBattleNetProxy->CV_Wait();
}
...
int main(int argc, char** argv)
{
	...
	if (!g_pBattleNetProxy->InitServerCfg(g_sCfgName))
	{
	...
	g_pBattleNetProxy->Start();

	MP_SYSTEM("%s NetProxy Final!", gServerName);

	g_pBattleNetProxy->DestroyInstance();
