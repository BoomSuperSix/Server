...
const char * gServerName = "MatchServer";
...
BOOL CALLBACK CosonleHandler(DWORD ev)
{
	...
	case CTRL_CLOSE_EVENT:
		g_pMatchNetProxy->Final("Close Event");
		g_pMatchNetProxy->CV_Wait();
		break;
	default:
		return false;
	}
...
{
	signal(sig, SIG_IGN);
	g_pMatchNetProxy->Final("Linux Signal Close Event!");
	//	g_pMatchNetProxy->CV_Wait();
}
...
int main(int argc, char** argv)
{
	...
	if (!g_pMatchNetProxy->InitServerCfg(g_sCfgName))
	{
	...
	g_pMatchNetProxy->Start();

	MP_SYSTEM("%s NetProxy Final!", gServerName);

	g_pMatchNetProxy->DestroyInstance();
